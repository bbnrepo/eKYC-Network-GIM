...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating ICICI Identities"
    ...
    infoln "Creating RBI Identities"
  ...
  while :
    do
      if [ ! -f "organizations/fabric-ca/sbi/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/icici/msp organizations/fabric-ca/icici/tls-cert.pem organizations/fabric-ca/icici/ca-cert.pem organizations/fabric-ca/icici/IssuerPublicKey organizations/fabric-ca/icici/IssuerRevocationPublicKey organizations/fabric-ca/icici/fabric-ca-server.db'
    docker run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/rbi/msp organizations/fabric-ca/rbi/tls-cert.pem organizations/fabric-ca/rbi/ca-cert.pem organizations/fabric-ca/rbi/IssuerPublicKey organizations/fabric-ca/rbi/IssuerRevocationPublicKey organizations/fabric-ca/rbi/fabric-ca-server.db'
